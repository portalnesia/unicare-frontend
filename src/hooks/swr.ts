...
export default function useSWR<D = any, Pages = false, Additional extends object = {}>(
	path: string | null,
	config: SWRConfiguration<ResponseData<D, Pages, Additional>> = {},
) {
	const auth = useSelector((s) => s.auth);
	const { get } = useAPI();
	const { fallback, ...rest } = config;
	const swr = nativeUseSWR<ResponseData<D, Pages, Additional>, ApiError>(typeof auth === "undefined" ? null : path, {
		fetcher: get,
